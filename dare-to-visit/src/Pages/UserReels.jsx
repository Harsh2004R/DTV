...
const UserReels = () => {
    ...
    useEffect(() => {
        const fetchPosts = async () => {
            try {
                const { data } = await axios.get(`${BE_URL}api/uploaded-reels/get`, {
                    headers: {
                            ...
                            >
                                <CardHeader  >
                                    <Flex spacing="4">
                                        <Flex flex="1" gap="4" alignItems="center" flexWrap="wrap">
                                            <Avatar
                                                name={el.uploadedBy.name}
                                                src={el.uploadedBy.profile_picture}
                                            />
                                            <Box>
                                                <CardHeading
                                                    size="sm"
                                                ...
                                                <Text fontSize={{ base: "12px", md: "15px" }} color="#03A9F4">
                                                    Uploaded: {new Date(el.createdAt).toLocaleDateString()}
                                                </Text>
                                            </Box>
                                        </Flex>
                                        <IconButton
                                ...
                                <CardBody
                                    ...
                                    <Text fontFamily="my" fontSize={{ base: "12px", md: "16px" }}>
                                        {el.comments?.[0]?.text || "Thanks for sharing we us..."}
                                    </Text>
                                </CardBody>
                                <CardFooter
                                ...
                                >
                                    <Button border={"none"} flex="1" variant="ghost" _hover={{ backgroundColor: "#000", border: "0.2px solid rgb(57, 99, 132)" }}>
                                        ...
                                        <Text ml="5px" fontSize={{ base: "xs" }} color="#fff">{el.likes || 0}</Text>
                                    ...
                                    <Button border={"none"} flex="1" variant="ghost" _hover={{ backgroundColor: "#000", border: "0.2px solid rgb(57, 99, 132)" }}>
                                        ...
                                        <Text ml="5px" fontSize={{ base: "xs" }} color="#fff">{el.comments.length || "No"}</Text>
                                    </Button>
                                    <Button border={"none"} flex="1" variant="ghost" _hover={{ backgroundColor: "#000", border: "0.2px solid rgb(57, 99, 132)" }}>
